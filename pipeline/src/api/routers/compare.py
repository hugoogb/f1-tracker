...
def _compute_driver_h2h(db, driver1_id, driver2_id, race_id_filter=None):
    """Compute head-to-head race results between two drivers."""
    r1_alias = RaceResult.__table__.alias("r1")
    r2_alias = RaceResult.__table__.alias("r2")

    filters = [
        r1_alias.c.driver_id == driver1_id,
        r2_alias.c.driver_id == driver2_id,
    ]
    if race_id_filter is not None:
        filters.append(r1_alias.c.race_id.in_(race_id_filter))
...
def compare_drivers(d1: str, d2: str, teammate: bool = False, db: Session = Depends(get_db)):
    ...
    head_to_head = _compute_driver_h2h(db, driver1.id, driver2.id)
    ...
    if teammate and teammate_race_ids:
        head_to_head = _compute_driver_h2h(db, driver1.id, driver2.id, teammate_race_ids)

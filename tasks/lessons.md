...
- **ruff needs `--force-exclude` in hooks**: `extend-exclude = ["alembic"]` in `pyproject.toml` only applies when ruff discovers files itself. A pre-commit hook passes explicit paths, which overrides the exclude, so every autogenerated Alembic migration fails the hook. `--force-exclude` exists for exactly this case.
